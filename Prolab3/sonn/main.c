     ...
     //tekrar:
      ...
      while(i<sutun)
      {
       ...
       if(giris->data==1){
        ...
        else if(giris->data==0 || giris->data==2) ///asagisi olmuyorsa
            {
             i=0;
             while(i<sutun)
                  {
...
{ //tekrar2:
  ...
  //  if(y1==0) {goto tekrar3;}
      ...
      if (giris->prev->data==1 && y1!=0 /*giris->prev->data!=NULL*/)
      {
        ...
        if(giris->next->data==4) {giris->data=3; y1++; giris=giris->next; goto q1;} /// sag
        else if(giris->next->data!=4) {i=0; while(i<sutun) {giris=giris->next; i++;} /// asagi
                if(giris->data==4) {
                i=0; while(i<sutun) {giris=giris->prev; i++;} giris->data=3; x1++;
                i=0; while(i<sutun) {giris=giris->next; i++;}
                }
                         //  else goto q;
        } //q:
        if(giris->prev->data==4) {giris->data=3; giris=giris->prev; y1--;} /// sol
        else if(giris->prev->data!=4) { i=0; while(i<sutun) {giris=giris->prev; i++;} /// yukari
                if(giris->data==4) {
                i=0; while(i<sutun) {giris=giris->next; i++;} giris->data=3; x1--;
                i=0; while(i<sutun) {giris=giris->prev; i++;}
                  }
        }
       }
...
for(i=0; i<=satir; i++){
    for(j=0; j<sutun; j++){
        matris[i][j]=rand()%2;
        if(i==satir) { matris[i][j]=3; goto m;}

        printf(" %d ", matris[i][j]);
        m: ekle (matris[i][j],satir ,sutun);
...
if (matris[x1][y1]==1)
{
    if(x1!=0 && x1!=(satir-1) && x1!=3)
    {
        if(y1!=0 && y1!=(sutun-1) && y1!=3)
